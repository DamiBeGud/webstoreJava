export function toast (alertType, message) {
  ...
  let existingAlert = document.querySelectorAll('.alert')
  if (existingAlert.length > 0) {
    bottomPosition = existingAlert.length * 70
  }

  alertDiv.className = `alert alert-${alertType} alert-dismissible fade show m-3`
  alertDiv.setAttribute('role', 'alert')
  alertDiv.style.cssText = `position: sticky; bottom: ${bottomPosition}px; left: 100%; max-width: 300px; height: 60px; border-radius: 10px; z-index: 1000;`
